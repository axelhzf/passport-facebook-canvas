  ...
  signedRequest.parse(function(errors, request){
    ...
    // First time app has seen this user, redirect user to 'failureRedirect'.
      ...
      self.userProfile( request.data.oauth_token, function(err,user) {
        ...
        var toVerifyUser;
        if (user && user._json) {
          toVerifyUser = user._json;
          toVerifyUser.signed_request = request.data;
        }
        ...
        try {
          if (self._passReqToCallback) {
            return self._verify( req, request.data.oauth_token, null, toVerifyUser, verified );
          }
          return self._verify( request.data.oauth_token, null, toVerifyUser, verified );
        } catch (ex) {
